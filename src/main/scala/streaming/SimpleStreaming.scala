...
/** Implement the streaming elements that the test-suite will verify
  *
  * Notice that in order to implement all those operations you do not need any
  * reference to an `ActorSystem` or `Materializer`. This is since the Flows /
  * Sinks / Sources are "descriptions", and actual execution happens only later.
  */
object SimpleStreaming extends ExtraStreamOps with SimpleStreamingInterface {
  ...
  /** Rather than re-using operations as `operation(source): Source`, let's
    ...
    *
    * Similar to function composition, Flows can be composed thanks to the `via`
    * operator, try to implement this method by composing the previous two.
    */
  ...
  /** You likely noticed that the `via` composition style reads more nicely
    ...
    * functions will be applied to the elements.
    *
    * Let's now re-use the passed in toStringFlow to re-implement the previous
    * source semantics, however by chaining multiple Flows with each-other.
    */
  ...
  /** Provide a Flow that will be able to continue receiving elements from its
    * upstream Source and "sum up" values while the downstream (the Sink to
    * which this Flow will be attached).
    *
    * In this way we are able to keep the upstream running it its nominal rate,
    * while accumulating all information. The downstream will then consume the
    * accumulated values also at its nominal rate, which in this case we know /
    * expect to be slower than the upstream.
    *
    * If the downstream would happen to be faster than the upstream, no such
    * aggregation is needed, and the elements can be passed through directly.
    *
  ...
  /** A faster downstream wants to consume elements, yet the upstream is slow at
    ...
    * by repeating the previously emitted value.
    *
    * This could be seen as a "keepLast", where the stage keeps the last
    * observed value from upstream.
    *
    * If you'd like to see the exact events happening you can call
    * `.logAllEvents` on the Flow you are returning here
    *
