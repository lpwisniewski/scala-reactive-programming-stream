...
class SimpleStreamingSuite extends munit.FunSuite with TestKitBase {
  ...
  test(
    ...
    val it = filterUsingPreviousFlowAndMapToStringsUsingTwoVias(ints, toString)
      .runWith(Sink.seq)
    val s = await(it)
    assertEquals(s, numbers(n).filter(_ % 2 == 0).map(_.toString))
  }
  ...
  test("moving average") {
    ...
    sourceProbe.sendNext(TimedValue(1, 0))
    sinkProbe.request(1)
    sinkProbe.expectNext(1)

    sourceProbe.sendNext(TimedValue(1, 1))
    sourceProbe.sendNext(TimedValue(1, 2))
    sourceProbe.sendNext(TimedValue(1, 3))
    sourceProbe.sendNext(TimedValue(2, 4))
    sinkProbe.request(1)
    sinkProbe.expectNext(1.2)
    sourceProbe.sendNext(TimedValue(5, 10))
    sinkProbe.request(1)
    sinkProbe.expectNext(5)
